(function () {
  ...
  const offsetKey = "smarttoc_offset";
  ...
  var mountRedraw = mountRedraw$1(render, typeof requestAnimationFrame !== "undefined" ? requestAnimationFrame : null, typeof console !== "undefined" ? console : null);
  ...
  var request = request$1(typeof window !== "undefined" ? window : null, promise, mountRedraw.redraw);
  ...
  var route = router(typeof window !== "undefined" ? window : null, mountRedraw);
  ...
  class StreamClass {
      ...
      static create(init, name) {
          const instance = new StreamClass(init, name);
          const callable = function (val) {
              if (arguments.length === 0) {
                  return stream$.value;
              }
              else {
                  assert(typeof val !== 'undefined', 'sending `undefined` to a stream is not allowed');
                  stream$.update(val);
                  stream$.flush();
              }
          };
      ...
      static combine(...streams) {
          const cached = streams.map((stream$) => stream$()); // could be undefined thought
  ...
  const Stream = Object.assign(StreamClass.create, 
  // provides type
  StreamClass, 
  // actually provides static methods
  Object.fromEntries(Object.getOwnPropertyNames(StreamClass)
  ...
  ***************************************************************************** */
  ...
  const calcStyle = function (options) {
      ...
      //-------------- y --------------
      scroller.dom === document.body ? 0 : scroller.rect.top;
  ...
  const ui = {
      render: (options) => {
          ...
          const $redraw = Stream.merge($isShown, $offset, $article, $scroller, $headings, $activeHeading, $topbarHeight).log('redraw');
  ...
  function activeHeadingStream({ scroller, $isShown, $content, $topbarHeight, addDisposer, }) {
      ...
      const $scroll = Stream.fromEvent(scroller === document.body ? window : scroller, 'scroll', addDisposer)
          ...
          .startsWith(null)
          .log('scroll');
      const $activeHeading = Stream.combine($content, $topbarHeight, $scroll, $isShown)
          .filter(() => $isShown())
  ...
  function contentStream({ $isShown, $periodicCheck, $triggerContentChange, article, scroller, addDisposer, }) {
      const $resize = Stream.fromEvent(window, 'resize', addDisposer)
          .throttle(100)
          .log('resize');
      const $content = Stream.merge($triggerContentChange, $isShown, $resize, $periodicCheck)
          .filter(() => $isShown())
          .map(() => {
  ...
  function topbarStream($triggerTopbarMeasure, scroller) {
  ...
  function createToc(options) {
      ...
      Stream.combine($isShown.unique(), $content.map((c) => c.article.height).unique(), $content.map((c) => c.scroller.rect.height).unique(), $content.map((c) => c.headings.length).unique())
          .map(([isShown]) => isShown)
          .log('readable');
      //-------------- Effects --------------
  ...
  if (window === getContentWindow()) {
      ...
      const renderToc = (article, headings) => {
          if (toc) {
              toc.dispose();
          }
